...
- **Foundation Models の4つの「無い」**:
  1. **窓が可変でない** — `SystemLanguageModel.context_size` は read-only の getter（native の `FMSystemLanguageModelGetContextSize` を読むだけ）。macOS 26.x 実測 **4,096**（8,192 は AFM 3 Core **Advanced** = M3/M4・12 GB 以上の機種のみ。M1 は macOS 27.0 でも 4,096、2026-09-22 実測）で、`use_case` × `guardrails` の全組合せで不変。**Ollama の `num_ctx` のような「低い既定値」ではなく天井**なので、送って広げることができない。超過は `ExceededContextWindowSizeError`。
...
- **M1 が載せる Foundation Models は AFM 3 Core（3B、窓 4,096）で固定** — SDK に `SystemLanguageModel.Variant.coreAdvanced3`（20B、窓 8,192）は存在するが `variant` は read-only で init に引数が無く、OS の model manager が機種で決める。Advanced は M3/M4・12 GB 以上限定で、M1 では「遅いが動く」でなく「存在しない」。capabilities は guidedGeneration / toolCalling / vision で reasoning 無し（macOS 27.0 / apple-fm-sdk 0.2.1、2026-09-22 実測）。
- **判定用途に要る 2 つが無い** — logprobs が無い。`fm serve` に prefix cache が無い（`cached_tokens` は常に 0、full prefill 1 回 約 7 秒）。同一 session の follow-up turn は約 2.3 秒だが transcript が 4,096 に当たるので約 16 turn が上限。Monte Carlo で分布を取る手も prefill 代で成立しない。
- **guided generation の schema は窓に算入され、書式が厳しい** — `title` / `x-order` / `additionalProperties: false` が必須で、54 key の schema が約 1,165 token を食う。`max_tokens` は `fm serve` が無視する（SDK の `maximum_response_tokens` は効く）。
- **同居の唯一の実利** — OS が生成中だけ約 2 GB を `TGOnDeviceInferenceProviderService` に確保し約 45 秒で返すので、gemma 常駐（5.9 GB）と同居しても swap は増えない。ただし下の「落とし穴」の判定品質の読みが偶然水準なので、同居できることは採用理由にならない。
...
- **prefix cache の再利用は `prompt_eval_count` でなく `prompt_eval_duration` で読む**。Ollama 0.34.2 は cache hit でも count に prompt 全体の token 数を返す（2026-09-22 実測: 同一 prompt の再送は 0.3 秒・`prompt_eval_duration` 0.21 秒でも count は 882 のまま）。per-option の logits 読みで「効いているはず」の見積もりと latency が合わないときはまずここ。判定は初回と以降の中央値の比（0.25）に、warm な daemon で初回も安い場合の絶対床（2 秒）を足す（`scripts/skillsel_arm_replay.py::prefix_cache_meta`）。
- **Qwen3.5 系（hybrid 線形注意）を判定役や prefix 共有の batch に使わない**。実測: Ollama で prefix が同じで suffix だけ違うリクエストが 7〜9 秒（同一 prompt は 0.3 秒）= 全量再評価、標準注意の qwen3:8b は 0.4 秒で再利用。torch/MPS では `flash-linear-attention` が CUDA 専用で reference kernel に落ち、kev-0.8b の 6,000 token リクエストが MPS 12.5 GiB で OOM した（2026-09-22）。原因が Gated DeltaNet の recurrent state にあることは**推測**（Ollama の runner で未確認）。切り分けは 7 コールの probe（同一 prompt ×2 / suffix 違い / logprobs 有無 / system 有無）で 3 分。
- **2 つ目のモデルは daemon の scheduler に任せず自分で降ろす**。`OLLAMA_MAX_LOADED_MODELS` 未設定の Ollama は公式 FAQ では「収まれば複数を同時ロード」（本機は未設定、同時ロードは未観測 — 依存しない側に倒す）。判定バッチの前に常駐モデルへ `{"model": m, "keep_alive": 0}` を投げ `/api/ps` が空になるのを待つ（gemma4:e4b の再ロードは約 7 秒。`scripts/skillsel_arm_replay.py::ensure_ollama_idle`、`core/llm/decision.py::_evict`）。
- **Foundation Models を Jev 型判定器（noul / choice / score）にすると偶然水準**（2026-09-22、M1・macOS 27.0）。RFC-0043 第 1 ラウンドの 20 行で、catalog を半分ずつ 2 コールに分けて数値を言わせる形（1 行 約 21 秒）の opus 参照ラベルへの AUC が 0.443（t=0）/ 0.466（t=1）、同じ行の gemma logits 読みは 0.736。seed 間の自己 Spearman 0.19、guardrail の `RefusalError` が 5/20 行、「0」の個数が行によらず 18/36 で一定 = 位置 artifact。skill ごとに 5 段階 enum を session 内で聞く形（5 行、1 行 約 147 秒）は AUC 0.61 だが 3/5 行で全部 "essential" に潰れる。logprobs も prefix cache も無いので Jev 型の確率読みは組めない — memory `apple-fm-m1-decision-arm-2026-09` が失効条件（M3 以降 / `coreAdvanced3` が available / SDK か fm serve が logprobs か prefix cache を出す）を持つ。
- **swap に沈んだ測定プロセスは待たずに作り直す**。別プロセスの全ディスク検索と重なった Laya の run は RSS が 784 KB に落ちて 1 行 200〜724 秒になり、kill → `--resume` で作り直すとロード 40 秒で 10〜20 秒の帯に戻った（2026-09-22、n=1）。in-process の arm は本番セッションの窓を自動では待たないので、本番 5 分前に止めて窓の後に resume する。
