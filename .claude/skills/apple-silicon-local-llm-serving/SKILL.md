...
- **prefix cache の再利用は `prompt_eval_count` でなく `prompt_eval_duration` で読む**。Ollama 0.34.2 は cache hit でも count に prompt 全体の token 数を返す（2026-09-22 実測: 同一 prompt の再送は 0.3 秒・`prompt_eval_duration` 0.21 秒でも count は 882 のまま）。per-option の logits 読みで「効いているはず」の見積もりと latency が合わないときはまずここ。判定は初回と以降の中央値の比（0.25）に、warm な daemon で初回も安い場合の絶対床（2 秒）を足す（RFC-0043 の撤去済み harness の `prefix_cache_meta`。復元手順は `docs/evidence/rfc-0043/README.md`）。
...
- **2 つ目のモデルは daemon の scheduler に任せず自分で降ろす**。`OLLAMA_MAX_LOADED_MODELS` 未設定の Ollama は公式 FAQ では「収まれば複数を同時ロード」（本機は未設定、同時ロードは未観測 — 依存しない側に倒す）。判定バッチの前に常駐モデルへ `{"model": m, "keep_alive": 0}` を投げ `/api/ps` が空になるのを待つ（gemma4:e4b の再ロードは約 7 秒。`core/llm/decision.py::_evict`）。
