...
### 「0 件になるルールなら意味がないのでは」への再検討

上の結論には**選定の順序に誤りがあった**。ルールを「今日 0 件になるか」で選ぶのは、
ゲートの意味を先に決めて証拠を後付けする順序で、drain コストの回避が価値判断を駆動していた。
正しい問いは「**示されたら実際に直す欠陥はどれか**」で、それなら drain を受け入れてでも
入れるべきルールがありうる。落としたものを測り直した結果、1 つ復活した。

**`no-mix-dearu-desumasu` は入れるべきだった。** 149 件という数字を「著者の文体と競合する」と
読んだのが誤りで、実際は **textlint の既定が規約と衝突していた**だけだった。既定は
本文=ですます / 箇条書き=である という別の意見を持つため、規約どおり全体を ですます で
書いた `README.ja.md` が 18 件上がっていた。文体の preference を外して**混在だけ**を見ると:

| 測り方 | 違反数 |
|---|---|
| textlint 既定 | 150 |
| 全体を である に固定 | 73（全て `README.ja.md`） |
| `README.ja.md` を ですます で測る | 0 |
| **preference を空にして混在のみ** | **0** |

tracked 107 本のうち 106 本が である で一貫し、`README.ja.md` だけが ですます で一貫していた
— コーパスは既に規約を満たしていた。「1 doc の中で文体を混ぜない」は好みではなく**明文化済みの
規約**なので、これはゲートに落とすべき対象（documented invariant → 決定論ゲート）。

一方、落としたままにした 2 つは再測定でも妥当だった:

- `sentence-length` — max=300 まで緩めても 109 件。密度の高い文はこのコーパスの文体そのもの
- `ja-no-mixed-period` — 195 件 / 75 ファイル。中身は「箇条書きに句点を打たない」という一貫した記法

**textlint 15.7 の `overrides` はルール option の上書きに効かない**（README だけ ですます に
切り替える形を files パターン 4 種で試したが全て 73 件のまま）。preference を空にする形は
その回避策であると同時に、意味的にもこちらが正しい — どちらの文体を採るかは doc ごとの
判断であって、ゲートが持つべきなのは「混ぜるな」だけ。

### 発火の実証

有効な 6 ルールすべてについて、違反を注入した canary を stage して staged mode が
exit 1 で止まることを確認済み（canary は撤去）。0 件は「ゲートが緩い」ではなく
「コーパスがきれい」であることの確認になっている。検出範囲の限界も記録しておく:
`no-mix-dearu-desumasu` が拾うのは `である` / `です` の明示コピュラで、
「〜だ。」「〜ている。」は拾わない。
