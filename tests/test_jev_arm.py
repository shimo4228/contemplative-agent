...
# --------------------------------------------------------------------------
# RFC-0045: the Score type and the relevance run
# --------------------------------------------------------------------------


def _relevance_payload(*, probabilities=None, noul=0.83):
    """A response shaped as the live API answered on 2026-09-24 (score + noul)."""
    return {
        "model": mod.DEFAULT_MODEL,
        "answers": {
            "score4": {
                ...
                "probabilities": probabilities or {"0": 0.1, "1": 0.1, "2": 0.2, "3": 0.6},
            },
            "noul": {"type": "noul", "noul": noul},
        },
        "usage": {"input_tokens": 378, "output_tokens": 33},
    }


class TestScoreQuestion:
    def test_a_score_question_carries_its_ordered_levels_as_criteria(self):
        question = mod.score_question("s", "How close?", ("low", "mid", "high", "top"))
        assert question.qid == "s"
        assert question.skill is None
        assert question.payload == {
            "type": "score",
            "instructions": "How close?",
            "criteria": ["low", "mid", "high", "top"],
        }

    def test_the_relevance_questions_are_the_replays_own_text(self):
        """Jev must be asked exactly what kev / von / gemma are asked."""
        rel = mod.load_relevance_module()
        questions = mod.build_relevance_questions(rel)
        assert {q.qid: q.payload for q in questions} == rel.systemone_request({}, "m")["questions"]

    @responses.activate
    def test_one_row_becomes_a_score_and_a_noul_entry(self):
        responses.add(responses.POST, mod.API_URL, json=_relevance_payload(), status=200)
        rel = mod.load_relevance_module()
        arms = mod.relevance_row({"domain": "d", "post": "p"}, _client(), mod.DEFAULT_MODEL, rel)
        score = arms[mod.RELEVANCE_SCORE_LABEL]
        assert score["probs"] == [0.1, 0.1, 0.2, 0.6]
        assert score["p_top"] == 0.6
        assert score["score"] == pytest.approx((0.1 + 0.4 + 1.8) / 3, abs=1e-6)
        assert arms[mod.RELEVANCE_NOUL_LABEL]["score"] == 0.83
        assert score["usage_input_tokens"] == 378

    @responses.activate
    def test_the_second_rate_limit_in_a_run_stops_without_waiting(self):
        """debugging.md: one Retry-After wait per run, the next 429 is a stop."""
        responses.add(responses.POST, mod.API_URL, status=429, json={})
        responses.add(responses.POST, mod.API_URL, json=_relevance_payload(), status=200)
        responses.add(responses.POST, mod.API_URL, status=429, json={})
        waited: list[float] = []
        client = _client(sleep=waited.append, max_rate_limit_waits=1)
