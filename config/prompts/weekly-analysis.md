...
- **Skills**: List all skills at period end. Note any added/removed/modified. The listing and diff cover `skills/*.md` only; retired skills live on under `skills/.archive/` and are excluded from both, so a "removed" skill is retired, not deleted — say retired.
...
- **Approval provenance** (from the **Approval provenance** block inside each state-diff section): every value-layer diff above is annotated with the in-window `logs/audit.jsonl` rows for that section (ADR-0012 gate; `ts` / `command` / `decision` / `source` / `content_hash`). Read it before writing the Identity / Constitution / Skills / Rules bullets, and state which of these three the section is — do not report a change as unverifiable when the block answers it:
  ...
  - **retirements present** — a line reading "N of the approved row(s) retired a file into `skills/.archive/`" means the gate archived N skills this week (ADR-0097 Decision 5: retirement is a move, never an unlink). Report them as approved changes alongside the adoptions, and note that the Skills bullet's "removed" entries are these. Those rows are deliberately excluded from the live-text reconciliation, so a retirement never appears as an approved-but-not-live finding — do not read their absence there as a discrepancy, and do not add "archived" to the causes of an orphan line that does appear.
