...
## 2026-09-19 土曜ゲート — 第 2 窓の読み（消費計画は満了）

`in_progress` 維持（採否は RFC-0041 の結論待ち）。第 2 窓を読んだ（`e12c09c`、表は
`docs/evidence/rfc-0014/skillsel-read-4-20260905.md` §5 の第 6 行）: 窓 09-05〜09-18、1,190 judged、
幻覚率 23.03% [20.7..25.5] — 帯 10〜25% の内。54 件レジーム単独（09-12〜09-18、602 judged）は
19.10% [16.2..22.4]。読みの後に archive 1 件で catalog は 53。第 1 窓は帯外（26.85%）、第 2 窓は
帯内 — 2 回が割れたので、消費計画の規定どおり結論は「帯では答えられない」で、3 回目は取らない。
天井仮説（catalog を減らせば幻覚率が下がる）は帯の読みでは当否が決まらなかった — 57 → 54 で
率は下がったが、同じ計器の日次の振れ（18.67〜35.06%）の内側の動き。
