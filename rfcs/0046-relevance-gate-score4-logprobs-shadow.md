...
**母集団の Jx（2026-09-26、evidence README「母集団の Jx」節）**: 本番定義（identity + axioms）で採点し直すと、記録 ≥ 0.82 の 1,042 行のうち分野外は 497（47.7%）→ **551（52.9%）**。分野外 → 分野内は 23 行、逆は 77 行、一致 0.904、Spearman 0.956。全層で Jx の方が厳しい。**「公理を教えれば分野内が増える」は母集団でも成り立たず、本 RFC の動機（過剰通過）はオーナーの定義の下で 53% として残る**。
...
1. **新定義の** answered 行が post_id dedupe で 150 に達したら（切替から約 2 日、2026-09-28 見込み — readiness の要約行で確認）`scripts/relevance_label_set.py sample` → `label`（opus、$ はオーナー承認）→ P(top) の precision / recall を t ごとに出し、t をここに書く（読みの前に固定）。noise floor は測った（Cx を dev 150 に 2 回、2026-09-26、evidence README「再現性」）: AUC の run 間差 −0.002 [−0.013, +0.010]（対 J）/ −0.007 [−0.018, +0.001]（対 Jx）、行単位 |ΔP(top)| 平均 0.023・p95 0.099、閾値近傍の gate 反転 150 行中 2〜4。**`score --baseline` の退行線は 0.02 でなく 0.03 に置く**（CI の外）。閾値は反転が少ない値を選び、face gate の一致率は ±2 pt を noise とみなす
