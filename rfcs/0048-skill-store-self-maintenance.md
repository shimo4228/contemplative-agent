...
- **skill の注入はコメントを変えるか**（2026-09-26 オーナー、open question）: 今の設計は gemma が選んだ skill の本文を
  全部生成の prompt に入れる（1 件 約 690 token、1 回平均 6 件 = 約 4,000 token）。入れると生成が良くなる / 変わる
  ことは直接測っていない。確かめ方の案: 同じ過去の状況で (1) 現行 (2) skill なし (3) 強い判定者（Jev か opus）の
  1 位 1 件だけ、の 3 通りで生成し、opus の対比較で読む。差が無ければ注入の機構そのものが見直しの対象になり、
  下の「件数を絞る方向」の前提も変わる
- 層 1 の規則に混同対を含めるか
...
## 2026-09-26 追記 — 退役と合わせて store の件数を絞る方向（オーナー指示）

出口の自己維持に加えて、**store の件数そのものを絞る**方向を本 RFC の設計対象に含める。根拠は RFC-0043 / 0044 の
測定で見えた gemma の skill 選択の弱さ（[evidence](../docs/evidence/rfc-0044/README.md)）:

- 選択は opus 天井と 0.14〜0.19 しか一致しない（sonnet 0.44〜0.48、Jev 0.35、opus 自己一致 0.68）。温度・enum 拘束・
  logits 読み・件数上限のどれでも動かない — 質は interface でなく判定者の力で決まる
- 7〜8 割の行で同じ skill を選ぶ。関連度の順位にはそこそこの信号がある（logits の AUC 0.73）が、集合に絞る段で崩れる
- 件数上限で絞ると関連度でなく出力順（名前順寄り）で切る（RFC-0044 S34）。**選ばせる側で絞るのは筋が悪く、絞るなら
  棚の側**
- 推測（未測定）: 53 件の名前は抽象的で似たものが多く（混同対 ADR-0105 は similarity 0.94〜0.96 の語形の揺れ）、
  約 3,000 token の説明から複数を選ぶ問いは 4B 級には重い。棚が小さく、互いに離れていれば選択は易しくなるはず

設計で決める点に足すもの:

- 目標の件数を数値で置くか（no-numeric-caps との関係 — 置くなら根拠と Review-when）、それとも「互いに区別できる」
  という性質で置くか（混同対・`duplicate` family の解消を退役 / 統合 / 改名で進める）
- 絞った効果の読み方: 同じ 150 行の標本で、件数を減らした catalog に対する gemma の選択と Jev / opus の一致を比べる
  （RFC-0043 の harness と Jev 行データが使える。天井側にも同じ件数条件を掛ける — RFC-0044 S34 の教訓）
- 上の open question（注入はコメントを変えるか）の答え次第で、絞る対象が「棚」から「注入そのもの」に変わる
