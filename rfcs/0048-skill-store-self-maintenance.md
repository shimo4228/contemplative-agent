...
`draft` → `blocked`（著者: 3a — 起票の指示で採用済み）。

- 再開条件: RFC-0042 の insight 再開後 3 回の土曜ゲートの読み（09-26 / 10-03 / 10-10 の run、現在 1 / 3）で、ゲートに届く候補が週 30 件台に戻っていない
- 照合先: `rfcs/0042-insight-entrance-narrowing.md` の判断役照合節と `logs/insight-stages.jsonl`
- 成立時: accepted（オーナーとの設計セッション → ADR）。30 件台に戻っていれば review-when どおり順序を読み直す

注記: Unresolved questions の「RFC-0021 を吸収するか」は、0021 が 2026-09-23 に `done`（出荷物 = ADR-0105 の混同対の読みは稼働中、天井仮説は未決のまま閉じた）なので、吸収でなく新しい問いとして扱う。
