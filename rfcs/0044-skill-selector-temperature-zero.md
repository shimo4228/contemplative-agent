...
## 2026-09-26 決定 — 第 2 段は見送り（オーナー）

`accepted` → `done`。第 1 段（temperature 0、幻覚 23% → 2.6%）で修理は済み、第 2 段（enum 拘束）は入れない。
理由: 上限なしの enum は選択数が増え（受入条件 (b) 不合格）、上限（maxItems）は関連度でなく出力順で切る
（S34 — 名前順に選択が歪み、退役判断の材料が歪む）。S34 の天井比較は opus 側に上限を掛けておらず条件が揃って
いなかった（オーナー指摘）ので、上限の小さい arm の質の根拠には使わない。Jev と件数を揃えた比較では、gemma の
1 件選択が Jev の 1 位と合うのは 7〜12%。S33 の実装（`bea29f1`）は main に入れず branch を削除、測定の読みは
[docs/evidence/rfc-0044/](../docs/evidence/rfc-0044/README.md) に凍結。gemma の skill 選択そのものが弱い件は
[RFC-0048](0048-skill-store-self-maintenance.md) の 2026-09-26 追記（store の件数を絞る方向）と、そこでの open
question「skill の注入はコメントを変えるか」へ渡す。
