...
オーナーが 2 判定を確定する。判断役の推奨: 判定 1 は成立として enum 拘束を別 RFC で起票（`parse_failed` と選択数の上限側を受入条件に入れる）、判定 2 は「文面上は成立だが測定に識別力が無い」と記録して蒸留へは進まない。確定後に RFC-0040 へ結果を 1 行戻し、script と `replay` group の撤去を決める。
