...
**Stats**: 30 modules, ~6900 LOC, Python 3.9+, 697 tests
...
| Layer | Modules | 責務 |
|-------|---------|------|
| **core/** (15) | llm, prompts, memory, episode_log, knowledge_store, distill, insight, rules_distill, scheduler, config, domain, report, metrics, _io | プラットフォーム非依存の基盤 |
...
| KnowledgeStore | JSON 配列 | 無制限 | effective_importance 順 top-K | バレットリストで注入 |
...
- **検索**: effective_importance 順で top-K をプロンプトに注入。参照時に last_accessed を更新

### 蒸留パイプライン (ADR-0008)

```
EpisodeLog (JSONL, 30件/バッチ)
  → Stage 1: LLM で自由形式パターン抽出 (DISTILL_PROMPT)
  → Stage 2: LLM で JSON 構造化 (DISTILL_REFINE_PROMPT)
  → _is_valid_pattern(): 30文字 & 3単語以上
  → Stage 3: LLM で importance 1-10 スコア付与 (DISTILL_IMPORTANCE_PROMPT)
  → _dedup_patterns(): SequenceMatcher で4分類
...
| **insight** | KnowledgeStore パターン | skills/*.md ファイル | 手動のみ |
| **rules-distill** | KnowledgeStore パターン | rules/*.md ファイル | 手動のみ |
