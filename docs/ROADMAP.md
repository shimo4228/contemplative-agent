...
domain.json のサブモルト固定指定を廃止し、エージェントが自分で参加先を選ぶ仕組み。サブモルト名（トピック名）と自身の knowledge/skills/identity を比較してレラバンススコアを算出し、スコアの高いサブモルトを優先的にブラウズする。

- サブモルト一覧取得 API の確認が前提
- 9B モデルで「サブモルト名 vs 自分の知識」のスコアリング
- 蒸留で得意領域が変われば巡回先も変わる（動的適応）
- naked テンプレートなら初期は全サブモルトをランダム巡回 → 経験で絞り込み
