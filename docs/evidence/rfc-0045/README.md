...
**noise floor（Cx を dev 150 に 2 回、2026-09-26 13:30〜14:00 JST、`relevance-noise-20260926.json`）**: argmax 一致 0.920、|ΔP(段 3)| 平均 0.023 /
p95 0.099 / 最大 0.253、閾値 0.3 / 0.5 / 0.7 での gate 反転 2 / 2 / 4 行。AUC の run 間差: 対 J −0.0017 [−0.0125, +0.0100]（0.9094 → 0.9078）、
対 Jx −0.0069 [−0.0177, +0.0009]（0.9623 → 0.9554）。row bootstrap 2,000 回。

### 母集団の Jx（2026-09-26、全 2,698 行、`relevance-population-jx-20260926.json`）

審判 J（domain = identity のみ）と Jx（domain = identity + axioms、本番の system prompt と同じ文）を同じ 2,698 行で比べた。線は P(段 3) ≥ 0.5。

| 行の集合 | n | J で分野外 | Jx で分野外 | 分野外 → 分野内 | 分野内 → 分野外 | 一致 | Spearman |
|---|---|---|---|---|---|---|---|
| logged ≥ 0.82 | 1,042 | 497（0.477） | 551（0.529） | 23 | 77 | 0.904 | 0.956 |
| logged ≥ 0.8 | 1,576 | 872（0.553） | 975（0.619） | 27 | 130 | 0.900 | 0.952 |
| 全行 | 2,698 | 1,902（0.705） | 2,049（0.759） | 32 | 179 | 0.922 | 0.955 |

層別（J → Jx の分野外率）: s0 ≤ 0.4 0.952 → 0.988、s1 0.5〜0.6 0.919 → 0.965、s2 0.7 0.843 → 0.886、s3 0.8 0.702 → 0.794、s4 ≥ 0.9 0.477 → 0.529。
全層で Jx の方が分野外を多く取る。J の 150 行の再採点分は dev の Jx 行を再利用し、残り 2,548 行は 2026-09-26 14:05〜14:15 JST に採点（約 $0.15）。
