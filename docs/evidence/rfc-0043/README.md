...
| kev-0.8b（MLX） | dev（判断役の補助読み、21:24〜21:26） | 分割 | 5 / 5（6 行目の前に打ち切り） | choice 2.2 秒 / noul 6.8 秒 | 5.7 → 12.0 GB（+6.3） | **打ち切り**（+6 GB の線。server の phys_footprint は再び 12 GB） |
...
| `K/choice/split`（kev-0.8b） | 0.306 [0.106, 0.524]（n 5） | +0.129 [−0.029, +0.288] | +0.087 [−0.109, +0.295] | 0.772 | — | 判定不能（5 行で打ち切り） |
| `K/noul/split`（kev-0.8b） | 0.210 [0.130, 0.324]（n 5） | +0.032 [−0.056, +0.132] | −0.009 [−0.195, +0.161] | 0.765 | — | 判定不能（5 行で打ち切り） |
...
kev-0.8b の 2 行は判断役の依頼で足した補助読みで、判定は変えない（資源の規則で smoke 止まりのまま）。5 行は dev の
先頭 5 行 = smoke と同じ行で、1 回の 5 行は証拠にならない。**数字の向きとして**、3 候補の中で kev-0.8b だけが
gemma を上回る側に平均が出た（choice +0.13、CI は 0 をまたぐ）ことだけを残す。
...
- **kev-0.8b の質**: 判断役の依頼で分割形の dev 30 行を回したが、5 行目の後に swap が開始時 +6 GB（5.7 → 12.0 GB）を
  超えて打ち切った。**30 行の読みは無い** — 「質でも届かない」とも「資源が解ければ候補に戻る」とも言えない。
  5 行の向きは gemma より上（上の dev 表）。30 行を揃える手は、server を数行ごとに作り直して MLX の確保を解放する形
  （swap は server 停止後も 12.0 → 7.7 GB までしか戻らなかったので、作り直しでも累積しうる）か、メモリの大きい機体
