...
| `skill_selection.py` | 472 | ADR-0076 shadow instrument: `select_applicable_skills()` pass-1 LLM pick over the skill catalog (name+description only, identity-only system prompt — no learned vocabulary fed back to the judge), `shadow_observe_skill_selection()` fire-and-log wrapper under `circuit_shield()` (never gates injection), audit → `logs/skill-selection-YYYY-MM-DD.jsonl`; `read_skill_selection_log()` / `format_skill_selection_report()` back `report --skill-selection`. ADR-0081 (accepted 2026-07-24, first shadow reading) decides enforcement — two-pass injection of only the selected skill bodies, flag-gated via `MOLTBOOK_SKILL_SELECTION_ENFORCE`; **implementation pending** (this row describes the shadow-only code currently shipped) |
