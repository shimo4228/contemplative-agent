...
| `llm.py` | 1049 | Ollama interface + `LLMBackend` Protocol (pluggable, returns `BackendResult`, keyword `temperature`, `model`/`context_window` properties), circuit breaker, sanitization, `drop_truncated` gate, backend-aware context-budget pre-flight (audit C2, ADR-0066), `validate_trusted_url` SSRF guard; `_build_system_prompt` reads identity.md as single blob (ADR-0030) |
