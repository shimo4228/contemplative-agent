<!-- Generated: 2026-08-01 | Updated: 2026-08-17 (added insight_surprise.py row and the ADR-0096 promotion-worth abstain path on insight.py) | Files scanned: 32 core modules | Token estimate: ~4161 -->
...
| `insight.py` | 983 | `extract_insight() → InsightResult`; global embedding clustering, no view batching (ADR-0050); ADR-0074 weekly-staged flow: `.last_insight` marker guard refuses an implicit full recluster, novelty gate delegated to `insight_novelty.py`, `--stage` writes a pending-review ledger consumed by `adopt-staged`. ADR-0096 promotion-worth abstain: `_extract_skill` returns an `ABSTAIN_*` reason code instead of a bare `None`, an in-band `NOTHING-PROMOTABLE` output is a verdict rather than an untitled fault, and `_worth_gate` (default on, `MOLTBOOK_INSIGHT_WORTHGATE=0` opts out, fails open with `reason=worthgate_*`) judges the produced skill. `nothing_promotable` is tallied apart from the four fault reasons (`FAULT_ABSTAIN_REASONS`), and that split also decides whether an empty run consumes the incremental window. Re-exports `skill_theme` (public name; ADR-0079 Phase 3a). |
...
| `insight_surprise.py` | 267 | ADR-0096 read-only surprise reading for insight candidates: `compute_surprise()` measures each cluster centroid against the `SURPRISE_REF_K` (1000) most recently distilled live patterns, masking the cluster's own members, and returns `s_mean` / `s_nn` with the raw reference distribution (`ref_cos_p50` / `ref_cos_spread`) attached as an ambiguity note. Ranks are positions within the batch, never a cutoff — nothing is dropped, deferred or reordered by a reading (ADR-0071 invariant 1). No z-normalization by design: the 2026-08-17 calibration measured a raw max-cosine spread of 0.108 at p50 0.806 that z-scoring inflates to ~5 sd. LLM-free; imports `knowledge_store` only. |
| `skill_selection.py` | 1058 | ADR-0076 shadow instrument: `select_applicable_skills()` pass-1 LLM pick over the skill catalog (name+description only, identity-only system prompt — no learned vocabulary fed back to the judge), `shadow_observe_skill_selection()` select-record-and-enforce wrapper under `circuit_shield()` (a failing selector can never trip the breaker for the generation it precedes), audit → `logs/skill-selection-YYYY-MM-DD.jsonl`; `read_skill_selection_log()` / `format_skill_selection_report()` back `report --skill-selection` (window aggregates + `SkillSelectionDay` per-day breakdown + `enforced_records` + per-name `never_selected_exposure` + `rejected_name_tally`, the last four added 2026-08-08 because window-wide aggregates hide regime changes and a bare hallucination *rate* hides which of three mechanisms produced it). `rejected_name_tally` carries each hallucinated name, its emission count, and the nearest catalog name by **surface** similarity (`difflib`, single-pass — `SequenceMatcher.ratio()` is not symmetric, so scoring via `get_close_matches` and recomputing would print a similarity that did not pick the winner). Surface rather than embedding on purpose: the embedding layer exists to see through vocabulary variation, and here vocabulary variation *is* the signal separating a wordform slip from a substituted word. The tally classifies nothing — near-1.0 / mid / low are the reader's buckets, not the instrument's (ADR-0071). Rendering is bounded at `_REJECTED_NAME_RENDER_LIMIT` rows with the remainder summarised, and the names themselves are withheld unless `format_skill_selection_report(..., include_rejected_names=True)` — see [architecture.md](architecture.md) Data Flow for why the default is restrictive. ADR-0081 enforcement (implemented 2026-07-24, same day as the deciding first reading; **unconditional since the 2026-08-08 amendment retired `MOLTBOOK_SKILL_SELECTION_ENFORCE`**): `shadow_observe_skill_selection()` returns the selected names (empty tuple = inject nothing) on a judged verdict, else `None` = full injection (fail-open, kill switch); every record carries `enforced`; `selected_skills_block()` builds the pass-2 bodies (skill_theme identity match + frontmatter strip + forbidden-pattern validation) consumed by `llm.build_system_prompt_with_skills()`. `configured_injection_regime()` (2026-08-08, ADR-0089 amendment) names what may reach `<learned_skills>` — `full_corpus` (kill switch: `audit_dir` unset) / `two_pass_selected` (selector configured) — derived from live module state in the same short-circuit order as `shadow_observe_skill_selection`; the third literal `full_corpus_shadow_observed` is unreachable after the flag retirement but survives for baselines that recorded it. It reports a **ceiling, not an outcome**: four further conditions (`empty_catalog`, `no_template`, `fail_open_llm`, `fail_open_parse`) still route an individual call back to full-corpus injection. `selection_preconditions_unmet()` covers the two deterministic ones for a preflight; `observed_injection_outcomes(audit_dir)` counts what calls actually did (`records` / `enforced` / `fell_back` / verdict tally, counts only — never the untrusted situation strings). Both consumed by the eval, which records the ceiling as `manifest.injection_regime` and the outcome as `injection_observed` |
| `rules_distill.py` | 416 | `distill_rules() → RulesDistillResult`; Practice/Rationale B-layer format (ADR-0048) |
| `stocktake.py` | 718 | Skill/rule audit: single-call LLM grouping (ADR-0046; skill pass groups on frontmatter description + Context summaries, rules pass on bodies — ADR-0046 amendment 2026-08-15; a no-verdict surfaces as `GROUPING_*` reason, never as "no duplicates"), `merge_group()` union-of-patterns, `CANNOT_MERGE` reject, singleton trigger-altitude clean (ADR-0048), `audit_skill_description()` usage/description audit (ADR-0081) |
| `report.py` | 321 | `generate_report()` JSONL → Markdown activity summary |
| `metrics.py` | 189 | Session metrics aggregation |
| `text_utils.py` | 214 | Shared Markdown helpers (`slugify`, `extract_title`, `split_frontmatter` / `strip_frontmatter`, `synthesize_frontmatter`, `context_summary`, `skill_theme` — frontmatter name/description reader shared by novelty gate / selector / stocktake, moved here 2026-08-15) and the single markdown-directory reader `read_markdown_documents` (`(name, raw, body)`; `read_markdown_bodies` is its projection) — ADR-0035 PR2, ADR-0048 |
