<!-- Generated: 2026-08-01 | Updated: 2026-08-29 (RFC-0016 — insight_surprise.py restored as an instrument only; insight.py 744→808L) | Updated: 2026-08-25 (skill_selection.py split: the pass-1 selector keeps the name, its two read-only instruments move to selection_metrics.py / never_selected_metrics.py over the shared selection_window.py) | Files scanned: 34 core modules | Token estimate: ~4161 -->
...
| Module | LOC | Purpose |
|--------|-----|---------|
| `_io.py` | 317 | `write_restricted`, `truncate`, `archive_before_write` |
| `run_context.py` | 34 | ADR-0078: mints process-wide `run_id` (uuid4) at import; `set_session_id`/`clear` for the active `run` session; read by `core/_io.py`'s `append_jsonl_restricted` to stamp every JSONL record |
| `config.py` | 46 | `FORBIDDEN_SUBSTRING_PATTERNS`, `VALID_ID_PATTERN`, `MAX_COMMENT_LENGTH` |
| `domain.py` | 406 | `DomainConfig`, `PromptTemplates` (reads `MOLTBOOK_HOME/prompts/` overrides with packaged fallback), constitution loader |
| `prompts.py` | 77 | Lazy-load proxy to `config/prompts/*.md` + placeholder resolution |
| `llm/` (package, ADR-0079) | 2121 | Ollama interface behind a **permanent facade** (`llm/__init__.py` — the public `core.llm` import path is API for siblings; transport + generation + `configure`/`reset_llm_config` live here). Submodules: `llm/backend.py` — `LLMBackend` Protocol (pluggable, returns `BackendResult`, keyword `temperature`/`think`, `model`/`context_window` properties) + circuit breaker + `circuit_shield()` context-manager isolation (ADR-0076: a failing observer can never trip the breaker for the generation it watches); `llm/prompting.py` — system-prompt assembly, sole owner of prompt-side mutable state, `system_prompt_budget_reading()` window-share instrument at adopt gate, `_build_system_prompt` reads identity.md as single blob (ADR-0030); `llm/guard.py` — sanitization, `validate_trusted_url` SSRF guard, `wrap_untrusted_content` (per-call nonce in the delimiter since 2026-08-16 — it defends the boundary's *position*, not any privilege; see architecture.md "Untrusted Boundary"), `strip_injection_tokens` (fixed-point, shared with `constitution.py`), and the `injection-detect-*.jsonl` liveness log wired unconditionally from `cli/runtime.py`. Generation keeps the `drop_truncated` gate, per-call `think` flag + reasoning-trace capture (`generate_for_api` returns `GenerationOutput(text, thinking)`); `llm/request.py` — frozen `GenerationRequest` (what the caller asked for) / `ResolvedRequest` (system prompt built + output budget clamped), so the resolution boundary is a type rather than a renamed positional argument, backend-aware context-budget pre-flight (audit C2, ADR-0066) |
| `clustering.py` | 137 | Average-linkage cosine agglomerative clustering via exact Lance-Williams merge (ADR-0074 perf rewrite, same partitions as the retired naive version), numpy-only. Used by `insight` and `rules_distill` |
| `embeddings.py` | 147 | Ollama `/api/embed` wrapper (nomic-embed-text), `cosine`, `embed_one`, `embed_texts` |
| `episode_embeddings.py` | 170 | `EpisodeEmbeddingStore` — SQLite sidecar for episode vectors (ADR-0019) |
| `episode_log.py` | 89 | `EpisodeLog` (append-only JSONL, `read_range` with `record_type` filter) |
| `knowledge_store.py` | 396 | `KnowledgeStore` — patterns JSON + provenance/bitemporal (ADR-0021); `is_live()` (bitemporal-only, `valid_until is None`; trust floor retired ADR-0051); `effective_importance()` (pure time decay `0.95^days`, LLM rating retired ADR-0056), `pattern_id()`, `epistemic_kind_for()`, `epistemic_counts_for()` (ADR-0050); `get_live_patterns()` / `get_live_patterns_since()` / `get_raw_patterns()` |
| `memory.py` | 289 | `MemoryStore` facade (delegates to `memory_repos.py`), `Interaction`/`PostRecord` dataclasses (`Insight` retired, ADR-0052) |
...
| `view_metrics.py` | 388 | Read-only pattern-composition instruments: consumed-view supply, seed-independent diversity (pairwise cosine + cluster structure), `nearest_view` singleton visibility (grounding composition removed — constant under per-episode mapping, ADR-0072). Observability only — never wired into gates/ranking (AKC ADR-0015 shape) |
...
| `insight.py` | 808 | `extract_insight() → InsightResult`; global embedding clustering, no view batching (ADR-0050); ADR-0074 weekly-staged flow: `.last_insight` marker guard refuses an implicit full recluster, novelty gate delegated to `insight_novelty.py`, `--stage` writes a pending-review ledger consumed by `adopt-staged`. ADR-0096 in-band abstain (kept by ADR-0097): `_extract_skill` returns an `ABSTAIN_*` reason code per non-yielding cluster, `nothing_promotable` (the extraction call's own `NOTHING-PROMOTABLE`) is tallied apart from the four fault reasons, an always-emitted yield line reports it, and the fault/verdict split decides whether `.last_insight` advances. The separate post-extraction worth judge stays removed by ADR-0097 (it read 46/46 promote on its first production run — its own pre-registered refutation), so extraction is still the only LLM call per cluster; the LLM-free surprise reading it was removed alongside was restored 2026-08-29 by RFC-0016 and is attached to each `SkillResult` by `_read_surprise` |
| `insight_surprise.py` | 301 | ADR-0096 read-only surprise reading for insight candidates, removed by ADR-0097 D1 and restored 2026-08-29 (RFC-0016 — ADR-0080's 2026-08-26 metabolic-quality amendment names the consumer whose absence was the only removal reason): `compute_surprise()` measures each cluster centroid against the `SURPRISE_REF_K` (1000) most recently distilled live patterns, masking the cluster's own members, and returns `s_mean` / `s_nn` with the raw reference distribution (`ref_cos_p50` / `ref_cos_spread`) attached as an ambiguity note. Ranks are positions within the batch, never a cutoff — nothing is dropped, deferred or reordered by a reading (ADR-0071 invariant 1), and the sidecar copy is display-only at the adopt gate. A candidate whose own material covers the entire reference window gets no reading rather than an unmasked one (2026-08-29 review) — which is every candidate under `insight --full`, so the reading is an incremental-run instrument; `ref_k` reports the post-mask sample size. No z-normalization by design: the 2026-08-17 calibration measured a raw max-cosine spread of 0.108 at p50 0.806 that z-scoring inflates to ~5 sd. LLM-free; imports `knowledge_store` only. |
| `insight_novelty.py` | 434 | ADR-0074 LLM novelty gate for staged insight: known-theme loading, token-budget chunk packing, covered-id parsing, novelty audit log (`skipped_known`). Extracted from insight.py (ADR-0079 Phase 3a); does not import insight. Re-exports `skill_theme` from text_utils (moved 2026-08-15). Namespace note: distinct from `adapters/moltbook/novelty.py` (feed novelty). |
| `skill_selection.py` | 534 | Pass-1 skill selection, write side only: `select_applicable_skills()` LLM pick over the catalog (name+description only, identity-only system prompt — audit H5), the audit append, and `configured_injection_regime()` — the single place naming what may reach `<learned_skills>`. Shadow-only at ADR-0076, enforcing since ADR-0081 (flag retired 2026-08-08). Reading the log back is an instrument and lives in the three modules below. |
| `selection_window.py` | 170 | Shared base for both selection-log instruments: day-file iteration, window resolution (`days` or an inclusive UTC `since`..`until`), tokenization. One implementation each, so the two readings cannot disagree about where a window is. |
| `selection_metrics.py` | 919 | Instrument (ADR-0071): per-window reading of `skill-selection-*.jsonl` — verdict mix, tokens saved, catalog-size regimes, hallucinated-name tally with its three-mechanism split. `format_skill_selection_report(include_rejected_names=…)` is a trust decision, not phrasing: rejected names are free model text and the default withholds them. |
| `never_selected_metrics.py` | 669 | Instrument (ADR-0097 D5): never-selected exposure in two scopes from one pass — strict (whole history) and dormant (trailing window). Feeds the weekly packet's retirement candidates; the floor is a listing criterion, never an automatic retire. |
| `stocktake.py` | 272 | Reduced by ADR-0097 to what survived the consolidator dissolution: `_check_skill_quality` / `_check_rule_quality` (deterministic structural checks; `run_skill_stocktake` / `run_rules_quality_check`), `audit_skill_description()` (ADR-0081 description-fidelity audit — the one LLM call left, advisory, untrusted-wrapped, one-line scrubbed reason), and `format_stocktake_report()` with the ADR-0081 usage section (never-selected with exposure). Grouping, union merge and the ADR-0048 clean stage are gone; duplicate structure is read from the selection log (co-selection families) and consolidation happens at the Saturday gate |
| `report.py` | 321 | `generate_report()` JSONL → Markdown activity summary |
| `metrics.py` | 189 | Session metrics aggregation |
| `text_utils.py` | 251 | Shared Markdown helpers (`slugify`, `extract_title`, `split_frontmatter` / `strip_frontmatter`, `synthesize_frontmatter`, `context_summary`, `skill_theme` — frontmatter name/description reader shared by novelty gate / selector / stocktake, moved here 2026-08-15) and the single markdown-directory reader `read_markdown_documents` (`(name, raw, body)`; its `read_markdown_bodies` projection went with `rules_distill`, ADR-0097) — ADR-0035 PR2, ADR-0048 |
| `thresholds.py` | 83 | Centralized thresholds with ADR/calibration annotations. `snapshot.collect_thresholds` reads from here. |
| `artifact_extraction.py` | 125 | Shared `extract_title → slugify → path-escape guard` chain (ADR-0035 PR3a) |
...
| `MAX_BATCH` | 10 | insight per-batch cap (`rules-distill`, the other consumer, retired by ADR-0097) | — |
