...
cli/ (package, ADR-0079)  -- composition root, only layer importing both core/ and adapters/
 ...
 |    wiki_maintainer.py (953L)  -- RFC-0017 S2 Maintainer (RFC-0022 shape): the day's rich episodes rendered once in chronological order, then consumed whole in budget-sized batches of ONE call each (the whole wiki in every prompt, re-read between batches), resume from the audit's batch rows, per-call JSON Schema whose op page-id enum is what is on disk, source intersection against the episodes that batch read, fail-closed reason codes, logs/wiki-maintainer.jsonl (run row + batch rows + b64 turn rows)
 |    wiki_loop.py (140L)         -- RFC-0017 S2/S3 shared loop mechanics: fail-closed classification (llm / parse / truncated), one constrained generation per turn, b64+sha256 turn audit rows. Schemas, dispatch and run records stay with each loop
 |    wiki_proposer.py (883L)   -- RFC-0017 S3 Proposer: four whole inputs (wiki index + skill index + evolution log + skill impact, no retrieval filter), fail_closed_budget rather than trimming, bounded open_page/open_skill/propose/abstain loop, one atomic proposal validated against catalog + opened pages + anchor uniqueness, rendered to wiki/proposals/ (never skills/), logs/wiki-proposer.jsonl
...
contemplative-agent wiki-propose [--dry-run] [--max-opens N] [--impact-days N]   -- one weekly iteration: reads the wiki index, the whole skill index, the evolution log and the skill-impact table, opens up to --max-opens pages/skills, and emits ONE atomic would-be proposal (new skill, or a patch to one existing skill) rendered under wiki/proposals/. Writes nothing into skills/ or .staged/ — the human gate is S6. Inputs that do not fit the window end the run fail_closed_budget rather than being trimmed
contemplative-agent wiki-maintain [--date YYYY-MM-DD] [--dry-run] [--catch-up-days N]   -- one UTC day (default yesterday), read whole: the day's rich episodes are consumed in chronological batches, one write/abstain call per batch with the whole wiki in the prompt, ops applied through WikiStore. A re-run of the same day resumes from the batch rows rather than re-reading (a day already read to the end is `already_done`, no LLM call). `--catch-up-days N` also reads the N days before yesterday, oldest first — the driver for days a previous run could not finish (plist default 2); after a `fail_closed_budget` the remaining days are `skipped_after_budget`. --dry-run calls the model and audits would-be ops without touching a page. Every LLM fault is a named fail_closed_* outcome and ends the day, never a silent no-op. Scheduled by `install-schedule --wiki-maintain` (04:15, after distill)
