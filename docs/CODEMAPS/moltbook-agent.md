...
contemplative-agent wiki-maintain [--date YYYY-MM-DD] [--dry-run]   -- one UTC day (default yesterday), read whole: the day's rich episodes are consumed in chronological batches, one write/abstain call per batch with the whole wiki in the prompt, ops applied through WikiStore. A re-run of the same day resumes from the batch rows rather than re-reading. --dry-run calls the model and audits would-be ops without touching a page. Every LLM fault is a named fail_closed_* outcome and ends the day, never a silent no-op. Scheduled by `install-schedule --wiki-maintain` (04:15, after distill)
