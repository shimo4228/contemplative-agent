...
Not every read-only script in `scripts/` is an intake. ADR-0097 added two that **no schedule invokes**: `coselection_families.py` (co-selection sibling / sub-case pairs and family any-of rates from the selection log, with minimum-support floors and Wilson intervals printed beside every conditional — D5/D7) and `retrieval_recall_measure.py` (offline lexical / cosine / RRF-union recall@k against the skills a weekly reviewer named as covering a rejected candidate — D6). The author runs them and reads the numbers; nothing renders them into a packet and no reason code of theirs reaches the recurrence channel. The distinction is load-bearing when reading this file: an intake's silence is a signal the chain produced, while these two are silent because nobody asked. Their inputs also sit behind the episode-log read boundary, so an agent cannot take the reading on the author's behalf. The never-selected reading (Stage 7b) is the counter-example that proves the split is about wiring rather than about the venv — it also runs under the venv, but it *is* wired, and its absence from a packet is therefore meaningful.
