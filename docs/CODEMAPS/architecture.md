...
The other four `claude -p` sites are unconverted and remain ambient-dependent (T-CHAIN-PERM-SWEEP); stage 4's fix session is the largest, being both write-capable and fed untrusted finding text, and its `Write(./**)` is inert — its real bound is the throwaway git worktree. The durable fix for the inherited allow list is config isolation, which needs credential provisioning first: an isolated `CLAUDE_CONFIG_DIR` starts unauthenticated.
