...
| Metric | Value |
|--------|-------|
| Total `.py` files | 88 (79 non-`__init__` + 9 `__init__`) |
| LOC | ~31049 |
| Test files | 100 (3764 tests collected) |
| Eval layer modules | 8 (`evals/` excl. `__init__.py`; outside `src/` and outside the wheel — measures LLM output quality, ADR-0089) |
| Core modules | 40 (platform-independent; 36 top-level modules + 4 in the `llm/` package) |
