...
3. **各エピソードをリッチに render（`render_episode`）。** ヘッダに続けて: `original_post`、`their_comment`（reply）、agent 自身の `content` と `title`（post 用）、`internal_note` を全文。各 external フィールドは新ヘルパー `truncate_boundary`（文末 → 単語 → 字、marker 付き）で、実測フィールド長の約 90 パーセンタイルに校正した `EXCERPT_CAP`（`original_post` 4700 字、`their_comment` 1500 字、`content` 4700 字）で excerpt する。`internal_note` は cap しない。
