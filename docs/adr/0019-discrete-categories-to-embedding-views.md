...
## Addendum (2026-07-11) — role evolution of the `self_reflection` view

As shipped here, a view was a *neutral* read-time lens: patterns were produced independently of any seed, and views only classified them after the fact. [ADR-0072](0072-echo-chamber-interventions.md) evolved the `self_reflection` view beyond that: the distill prompt (`config/prompts/distill_episode.md`) now instructs generation toward a first-person, moment-indexed register (write side), and the view seed describes and exemplifies the same register (read side). "Views store the questions" still holds, but this particular question doubles as a generation target — a production→retrieval contract. Edit prompt and seed as a pair; the view-supply instrument (`core/view_metrics.py`) is the drift detector. The `constitutional` view has no such coupling (its seed is the live constitution itself via `seed_from`). The role boundary between views and prompts, and the rule for when a pipeline gets a view versus unsupervised clustering, are documented in the `core/views.py` module docstring and ADR-0031's consumption note.
