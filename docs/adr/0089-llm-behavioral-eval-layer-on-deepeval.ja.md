...
> **注記（2026-09-19, 下の 2026-09-19 Amendment）**: 上段の「はるかに稀」という見積もりは
> 誤りだった。RFC-0042 は 2026-09-12 baseline に対して registry template を 8 本
> （insight 系）動かし、まさにこの誤検知を生んだ。face 限定案は依然として採らず、
> 応答は人間の確認記録にした。
...
## Amendment (2026-09-19): eval に見えないプロンプト変更は、測り直すのでなく確認して記録する

`prompt_templates_sha256` は `PromptTemplates` の欄を持つ `config/prompts/*.md` を
すべてハッシュする。2026-08-08b の Amendment は生の glob からこのレジストリへ範囲を
絞り、そこで止まった —— 当時レジストリは「エージェントが読む template」の妥当な代理
だった。しかし「*eval* が読む template」の代理ではない: eval が測る経路は
`generate_comment` 1 本だが、レジストリは蒸留・アイデンティティ・insight の template も
抱えている。

RFC-0042 でこの差が運用に現れた。insight の入口を段の列に作り直した際（ADR-0111）、
`comment_golden-2026-09-12` baseline に対して registry template が **8 本**動いた ——
`insight_extraction.md` と `insight_novelty.md` が編集、6 本が新設
（`insight_description` / `insight_duplicate` / `insight_duplicate_system` /
`insight_name` / `insight_naming` / `insight_naming_system`）。どれも測定済みの verdict を
動かしうる経路にないのに、承認済み baseline は STALE を報告し、eval 1 回分を払って
再承認せよと指示した。

これは 2026-08-08b の Amendment の予測を反証してもいる。あの Amendment は残る誤検知を
「今回閉じたものよりはるかに稀 —— script 読みの文書は週次パイプラインの作業で編集されるが、
distill の template はされない」と値付けした。6 週後、同じ類の insight template が 1 つの
PR 群で 8 本動いた。該当節には注記を立てた。

### 検出は広いまま、逃げ道を人間側に置く

代替案を 3 つ検討した。

**測定対象の face がロードする template だけをハッシュする。** 2 度目の却下である。
2026-08-08b「本変更が直さないもの」で既に「第 3 の選択肢であり、採らなかった」とされて
おり、理由は「導出元の registry が無いので 1 face に紐づく手書きリストになり、生成経路が
変わるたび・2 つ目の face が入るたびに改訂が要る」だった。この理由は今も立つ。本 Amendment は
1 点を足す: 2026-08-08b の allowlist が許されるのは、新しいファイルを PR 時に機械が
どちらかのバケツへ強制的に入れるからである（`tests/test_packaged_assets` の orphan guard）。
「`generate_comment` に届くか否か」には安い番人が無い —— プロンプトレジストリを介した
呼び出しグラフについての主張であり、そこでの分類ミスは検出漏れの向きに沈黙する。

**検出はそのままにして、変更ファイル一覧を印字するだけにする。** 警告はそもそもブロック
しないので、情報つきの誤検知で十分ではないか —— 読者は `insight_*.md` を見て肩をすくめ、
先へ進む。却下の理由は読む側のコストではなく `--baseline` の流れにある。`compare.py` が
同じ欄で diff を拒む（exit 2）ため、insight プロンプトの編集から次の再承認までの間、
使える regression gate が存在しない —— 本物の regression を捕まえるはずの run が比較でき
ない。肩をすくめてもそれは戻らないが、確認の記録なら戻る。

**manifest にファイル単位のダイジェスト表を持つ。** 将来の baseline が合成ダイジェスト 1 本
でなく `{path: sha}` を保存すれば、「どのファイルが動いたか」に git 無しで答えられ、将来の
絞り込みを人間の免除でなく欄単位で行える。今は採らない: manifest schema が変わり、既存
baseline の比較可能性をすべて無効化し、しかも本 Amendment が必要とするものは何も買わない
（一覧は今日 git から取れる）。確認が頻発するなら自然な次の一手として残る（下の再検討条件）。

そこで見逃しを機械から外し、人間の前に置く:

そこで見逃しを機械から外し、人間の前に置く:

- `check_staleness.py --acknowledge --reason TEXT` が成功するのは、乖離が
  `prompt_templates_sha256` **だけ**のときに限る。他の欄 —— target model / temperature /
  dataset / assets / judge prompt / sampling / injection regime —— が 1 つでも乖離して
  いたら、その欄を名指しして拒否し、何も書かない。それらは run が実際に別物を測る
  ことを意味する欄で、確認で免じられるものがない
- `--dry-run` を付けると全検査と変更ファイル一覧の印字だけを行い、何も書かない ——
  記録の前に証拠を見たい読者のため
- 記録先は baseline の隣のサイドカー `<baseline-stem>.ack.json` への追記で、UTC タイム
  スタンプ・両方のダイジェスト・理由・baseline の commit 以降に変わったプロンプトファイルの
  一覧を持つ。`--reason` は必須 —— 中身が「誰かがボタンを押した」でしかない記録は記録では
  ない。何も書かない呼び出しが 2 つある: baseline が fresh のとき（「確認するものが無い」）と
  既に記録済みのダイジェストのときで、どちらも exit 0。拒否は exit 1 で STALE と同じ
  コードである —— 拒否は baseline を stale のまま残すからである
- **baseline 本体は 1 バイトも書き換えない。** baseline は測ったものの記録であり、確認は
  スコープについての判断である。新しいダイジェストを baseline に書き込めば、存在しない
  測定を捏造することになり、次の読者は両者を区別できない。`check_staleness` は
  どちらなのかを言い（「測定でなく確認による一致」、日付と理由つき）、`compare_runs` は
  確認済みダイジェストを呼び出し側が baseline ファイルから解決して渡す明示引数として受ける
- 確認は**両方**のダイジェストに束縛される。これは pair についての主張
  （「この baseline のダイジェストからあのダイジェストへの差分は範囲外」）なので、
  baseline が別のダイジェストを持っていた時期に記録された項目は何も免じない。これが無いと、
  同じファイル名で baseline を in-place 再承認したとき、旧ファイルの免除を黙って
  引き継ぐことになる（security review で発見）
- `compare.py` も同じ確認を尊重する。これが無いと逃げ道は半分しか通らない:
  警告は黙るのに、次の本番 run は「問題ない」と言われた baseline との diff を拒む
- 壊れたサイドカーは cannot check（exit 2）であり、fresh でも stale でもない ——
  読めない記録に対してこの層が一貫して使っている規律と同じ

### 変更ファイル一覧は baseline でなく git から出す

baseline は合成ダイジェスト 1 本しか持たず、どの template が動いたかは言えない。git は
言える。一覧は「baseline を追加した commit」からの `git diff` を `config/prompts` と
`config/domain.json` に限って取り、現在のダイジェスト入力へ絞り込む（加えて、既に存在
しない path も残す —— 人間が確認を信じるかどうかを判断する場面では、多めに出す側が安全）。
比較先は **working tree** である。警告を出したダイジェスト自体が working tree から
計算されているからだ。git が答えられないとき（repo でない / git が無い / baseline が
未 commit）は一覧を `"unknown"` として記録し、空リストにはしない ——「見られなかった」と
「何も変わっていない」が同じに読めてはならない。

### 何を手放したか

不注意な確認は、コメント経路の template への本物の変更を次の再承認まで隠す。これは
従来設計に無かった新しい見逃しである。抑えるものは 3 つあり、どれも機械検査ではない
（上記の pair 束縛は機械検査だが、閉じている失敗は別物 —— baseline をまたいだ継承であって
不注意な免除ではない）:

1. 変更ファイル一覧は記録の項目に入る。主張がハッシュでなく名前の隣に綴じられる。
   既定の呼び出しでは印字と書き込みが同時に起きるので、一覧を読むことは書き込みの
   ゲートではない —— この bound が宿るのは「瞬間」でなく「記録」の側である。
   答えを先に見たい読者のために `--dry-run` があるが、opt-in であって確認ステップではない
2. サイドカーは commit される。主張と理由が diff とレビューに、免じた baseline の隣に残る。
   commit を強制するものは無く、README が求める習慣であって性質ではない
3. 範囲は 1 欄のみ。run が実際に測るものは依然としてすべて再実行を強制する

繰り返し払うコストもある: 1 項目が覆うのはダイジェスト 1 個ちょうどなので、insight
プロンプトを編集するたびに新しい STALE が立ち新しい項目が要り、baseline を再承認すれば
一覧は振り出しに戻る。分類表を作らないことの代金である。

代替案は同じ向きでより悪い: 動かしようのない変更で stale を出す baseline は、読者に警告を
無視する訓練を施し、無視された警告は**すべて**を沈黙のうちに見逃す。この ADR の
Amendment 群が繰り返し戻ってくる失敗様式がそれである。

### 再検討条件

この逃げ道は機構であり、ここでの機構は溶解計画を負う
（[ADR-0101](./0101-instrument-dissolution-mandate.md)）。次のどちらかで再検討する:
(a) 単一 baseline に確認が **3 件以上**積もったとき —— 免除が常態化したという読み値であり、
上記のファイル単位ダイジェスト表が schema 変更に見合う; (b) 2 つ目の eval face が入ったとき
—— 「どの face がこの template を読むか」は manifest が答えねばならない問いになる。
ダイジェストが欄単位になる設計が入れば、免じる対象そのものが無くなるので逃げ道は退役する。

### 触らなかった配線

`.claude/verify.sh` は変更しない。staleness ブロックは `check_staleness.py` の出力を
**exit が非 0 のときに**そのまま印字するので、新しい案内（プロンプトダイジェストが唯一の
乖離のときだけ出す）は承認ハッシュで固定されたゲート script を編集せずに advisory へ届く。
同じ配線の帰結として、確認が成立した後は検査が exit 0 になり verify は何も印字しない ——
「測定でなく確認による一致」の行は、検査を直接叩いた人にしか見えない。常用の読者はゲートが
免除で通っていることを告げられず、それを告げるのは diff の中のサイドカーである。可視化には
承認ハッシュで固定されたゲート script の編集が要り、それは人間の操作であって本変更の
範囲ではない。
