...
6. **LLM 採点の退役に向けた実測ゲートを設ける。** ablation の証拠 (`docs/evidence/adr-0053/importance-ablation-20260606.md`) は事前登録した「差が小さい」基準を満たし、純粋な時間減衰を残して distill 時の LLM 採点を退役させることを支持する。退役の決定自体は二つの条件が揃うまで保留する: (a) §B1 閾値 retune 観察窓 (relevance ゲートは 2026-06-05 に retune) が閉じること、(b) AKC position paper が出ること (AKC ADR-0003 の Layer-2 仕様が現在 importance score を名指ししており、論文化の最中に退役させると仕様が揺れる)。決定の前に ablation スクリプトを再実行すること — corpus は成長し、結果は変わりうる。
...
- AKC P1-5 の昇格判定は保留のまま (WON'T DO が維持される)。再判定のトリガーは、本 ADR の安定、Decision 6 のゲートの解決、AKC 論文の出版である。仮に昇格するなら候補となる内容は: extract 時の一回限り付与 / immutable store / read 時減衰 / gate-vs-score の区別 / re-extraction による昇格 — 「検索の重み」という表現は落とす。score の消費先は substrate 固有だからである。
