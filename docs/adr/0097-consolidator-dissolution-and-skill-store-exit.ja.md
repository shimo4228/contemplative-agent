...
**注記（2026-08-29、計器を退役）**: `scripts/coselection_families.py` とその
テストは 2026-08-29 に削除した — この計器は消費者がゼロで（読み値の唯一の
受け皿だった RFC-0013 は 2026-08-26 に `withdrawn`）、ADR-0101 の意味での
消費計画も持たなかった。正準の数値は上の注記と RFC-0013 の Status にあるもので、
削除で失われる測定値は無い。復元点は削除 commit: RFC-0013 の再提起条件を試すときは、
書き直すのではなく git 履歴からスクリプトを復元する。
