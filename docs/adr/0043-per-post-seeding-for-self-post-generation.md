...
- Per-voice fragments may appear near-verbatim in the agent's generated content (the prompt explicitly says "Stay close to the specific language each voice uses"). The generated content is logged at INFO in `agent-launchd.log`. This was implicitly true under the pre-ADR-0043 LLM-summary path too, but is more likely now. If Moltbook post bodies ever contain PII the agent does not control, this becomes the surfacing locus.
  - **2026-08-01 update (T-LOG-DEBUG-CONTENT)**: the surfacing locus was wider than "INFO". The production `com.moltbook.agent` plist ran with `-v`, so `log_published` also emitted the *full* body at DEBUG into the same file, and `reply_handler` dumped 200 characters of raw notification JSON — other agents' text, not just near-verbatim fragments of it. Because `scripts/log_anomaly_sweep.py` globs `*.log` and `scripts/weekly-analysis.sh` feeds its output to `claude -p`, this was a side channel back into the weekly prompt that [ADR-0083](./0083-episode-logs-enter-the-weekly-prompt-as-hashes-only.md) had closed for episode logs. Fixed by removing the DEBUG body branch outright (its `full_fmt` / `full_args` parameters no longer exist), reducing notification logging to key names, dropping `-v` from the plist template, and gating both in `tests/test_publish_logging.py` and `tests/test_cli_schedule.py`. The INFO preview described above (`log_preview`, 80 chars, single line) is unchanged and remains the surfacing locus this consequence names.
