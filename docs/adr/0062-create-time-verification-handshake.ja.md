...
2026-07-25 第10 amendment: 構造のみ — 文法は不変。`verification_parse.py` の 2 つの解決分岐
（明示的な演算子チェーンの `_resolve`、guarded な 2 オペランド読みの `_resolve_implicit`）は、
最後のオペランドの後に何が続くかを生イベント列からそれぞれ別々に導出しており、その 2 つの導出は
すでに 2 回ドリフトしていた（非隣接マーカーのノイズ規則、subtract 対 "combined" の矛盾ガード —
どちらも片方の分岐にしか実装されておらず、レビューで発見される前に server-rejected な答えを
生んでいた）。両分岐は 1 つの `_TailSignals` を読むようになり、3 回目の乖離は「起きにくい」ではなく
構造的に不可能になった。検証は新設の差分ゲート
`docs/evidence/adr-0062-parser-rewrite/differential_replay.py` — 凍結した改修前パーサ
（`verification_parse_baseline.py`）と現行を監査コーパスの全ユニーク challenge に流し、abstain 込みで
全件一致を要求する（この時点で 2149/2149）。挙動不変の変更では ground truth は無関係で、問うべきは
「旧コードが決めたことを新コードが正確に決めるか」だけなので、ラベル付き ~82% ではなく全コーパスを
対象にする。同時に観測された別件（本改正では未対応）: ground-truth ゲート `replay_parser.py` は
server truth に対し誤答 7 件で FAIL し、それは改修前 commit でも同一。コーパスは第 8 amendment 時の
1272 から 2149 に成長しており、その間に蓄積した新しい文法失敗である。解消は第 11 amendment であって
リファクタではない。

2026-07-25 第10 amendment その 2: 構造のみ — 文法は不変。第10 amendment が除いたのは分岐間の
*重複* であり、こちらは重複を作りやすくしていた構造そのものを除く。`_resolve` / `_resolve_implicit`
は 26 / 27 分岐の命令的な if/return 連鎖で、次の改正をどこに足すかは形から読めなかった。分岐数は
不変で、減らすことは目標ではない — 9 回の改正で実際の server-rejected 応答から削り出した文法規則
そのものだからである。変えたのは表現で、順序つき規則表 2 つ（`_EXPLICIT_TAIL_RULES` 4 行、
`_IMPLICIT_RULES` 14 行 = 元の `return` 1 つにつき 1 行）を単一のドライバ `_resolve_operation` が
上から評価する。各行は「名前 + 述語 + 結果」で、次の改正は行を足すだけになり、述語の列がどこに
足すかを示す。

設計上、効いているのは 2 点:

- **判定型が三値である。** abstain した規則と適用外の規則は下流ではどちらも `None` に見えるため、
  素朴な `... -> str | None` の表では abstain したガードが次の規則へ落ちて、文法が「黙れ」と
  言っている入力に答えてしまう。`_Decision.stop` がこれを分ける。9 回の改正の規則の大半は
  abstain なので、この潰れは稀な事故ではなく大半に効いたはずである。`_compute_chain` の域外
  `None` を `_answer()` で包むのも同じ理由 — 発火して答えの無い規則も停止する。
- **すべてを表にしたわけではない。** `_resolve` 冒頭の位置分類（イベントを gap / tail に畳み込み、
  collapse し、曖昧性を判定する）は決定カスケードではなく畳み込みであり、その abstain は分類自身の
  ガード節で、ループがまだ組み立て途中の context に対する述語にはできない。`_classify_positions`
  として切り出し、理由を docstring に書いたうえで命令的なまま残した。規則は配置が何を*意味するか*を
  決め、この関数は配置が何で*あるか*を決める。

入れ子条件の平坦化はガードが黙って適用範囲を広げる箇所なので、外側の条件はすべて名前つきの
context フィールドにし、平坦化した各行がそれを再度述べる形にした。各ブロックは自分のシグナルに
対して網羅的である。context は eager に導出するが、関与するヘルパーはすべて純粋・非 raise・
有界（敵対的入力に対する DoS の問いに答えるのと同じ有界性）なので、観測上は等価である。

1 行だけ素直な転記ではない: 隣接乗法 override は従来チェーンを `*` に書き換えてから subtract 対
"combined" ガードへ落ちていたが、override 後は乗算が減算になることはないのでそのガードは不活性で
あり、行が直接答えを返す。読んで確かめるものではなく差分リプレイが証明するものなので、その旨を
行にコメントとして書いた。

検証は第10 amendment と同じ差分ゲート（凍結ベースラインと 2149/2149 一致）に加え、cross-model
レビューが独自に 20,000 件のランダム challenge を新旧両版へ流して差分ゼロを確認した。
`replay_parser.py` は誤答 7 / abstain 382 / correct 1755 のまま — 変わらないことがここでの要点である。

`_resolve_operation` は規則表が網羅的でない場合、abstain ではなく raise する。この raise は
`code_parse_challenge` の `except _Abstain` では捕まらないため、網羅的でない表は untrusted な
CAPTCHA テキストを、文書化された fail-closed な abstain ではなく呼び出し側のクラッシュに変える。
この不変条件はコメントではなくテスト（`TestRuleTableTotality`）になり、終端行を一時的に条件付きに
して発火を実証した。

第11 amendment 2026-07-26: 第10 が先送りにした文法変更。ground-truth ゲートは 2026-07-15 から
FAIL しており、その失敗は corpus とともに増えていた — unique 2149 で誤答 7、11 日後の unique 2236
で 9。corpus はライブトラフィックであり、古い誤答が放置される間に新しい文法失敗が積み上がるためで
ある。9 件はいずれも `code_parse` の監査レコード 1 件のみで、サーバが "Incorrect answer" を返した
もの。退役した `llm_reason` 経路の残渣は 1 件もない。

うち 6 件が文法、3 件はそうではなかった。**文法側の修正はすべて abstain である。** 新しい読みは
足していない — どの形も 2 通りの読みが実証されており、サーバがどちらを求めるかを corpus は語らない
からで、「誤った parse は `None` より悪い」という既定の規律がそのまま決める:

- **加算 cue を集合として読む。** `_TailSignals.contradicts_subtraction` は `_ADDITIVE_CUES` が
  3 語持つのに `combined` の 1 語だけを見ていたため、`total` で cue された減算チェーンが、まさに
  それを止めるために作られたガードの脇を素通りして 17.00 を submit していた。シグナルを 1 箇所で
  導出すること（第10 amendment の修正）は、1 箇所で語彙化することと同じではなかった。両分岐が同じ
  property を読むので両行が同時に閉じる。corpus 全体で減算チェーン + cue は 3 件しかなく他 2 件は
  既に abstain していたため、coverage の損失はゼロ。
- **連結詞で断ち切られた長いチェーンは abstain する。** `_resolve` の文法は operand と operation が
  厳密に交互であることを要求する。operand が 3 つ以上のチェーンの gap にある `and` は節境界であり、
  2 つの文が 1 つの畳み込みに接着されていることを意味する。サーバに却下された長チェーン 2 件は
  どちらもこれを持ち（3703.00 は場面の速度を明示された積に畳み込み、3920.00 は重複した量を
  amplifier チェーンに畳み込んだ）、corpus で唯一正しい 3-operand チェーン `453e7b97` は持たない。
  この述語は閾値を持たない — 同じ corpus で同点だった gap 幅キャップより優先した理由がそれである。
- **単位名詞句の内側の `*` は演算ではなく句読点である。** 難読化は記号をどこにでも落とし、量と
  その単位名詞の間にも落とす。"thirty two `*` newtons and another sixteen newtons ... total?" は
  積の句読点をまとった加算であり、記号を演算として読んだ結果 48.00 の問いに 512.00 を submit して
  いた。手がかりは語彙ではなく位置 — 記号の次の atom が、もう一方の operand に続くのと同じ単位
  名詞である。corpus で受理された `*` + `and` の challenge では、記号は常に単位名詞の後ろに来る。
  `*` 限定は意図的で、corpus には単位句の内側の `+` が 1 件（`fe8bb27e`）あり、そこでは加算読みと
  implicit-add 読みが一致して受理されている。迷い込んだ `*` は答えを反転させるが、`+` は反転させない。
- **壊れた tens+unit 複合語は parse を poison する。** `_scan` は atom 単位で merge し、一致しな
  かったものを捨てるので、2 語数詞の片側が壊れても大きな音を立てて失敗しない — 消えて、残った側が
  operand になる。誤答 2 件がこれ由来だった: `treee` は `tre`（3 文字）に collapse し全 fuzzy tier
  の下を通るため、23 - 7 のはずが 20 - 7 = 13.00 になった。`trween` は `trwen` に collapse し、
  全 canonical / collapsed 数詞から距離 2 — fuzzy tier をどう広げても届かない — ため、分割された
  unit 半分だけが残り 3 + 5 = 8.00 が出た。これは round 8 の near-miss poisoning のワンサイズ下で
  ある。2 つの腕は同じだけ裏づけられているわけではなく、コードもそう書いてある: 第 1 の腕は tens 語の
  存在を確認したうえで隣の残骸を判定するが、第 2 の腕はそれができない — `trwen` が tens 語だった
  ことを証明するものは何もない — ので、残された輪郭（長い未一致残骸の後ろで challenge の先頭に立つ
  分割された 1 桁 operand）を、corpus で調整した 4 つの連言で拾う。どれを外しても実際の正答を失う:
  第 1 operand 制限を外すと 3 件、1 桁の上限を外すと 53 件、2 文字断片を許すとさらに 9 件。
  3 文字床がここで安全なのは、ガードが 1 つの位置 — 複合語のもう半分が入るべきスロット — に
  縛られているからにすぎない。全体に適用すれば `the`（collapsed `three` から 1 edit）で発火して
  corpus の大半を沈黙させる。`the` / `ton` / `tons` を fuzzy stopwords に加えたのも同じ理由で、
  `_match_fuzzy` に対しては no-op であることを検証済みなので coverage コストはゼロ、ガードの
  爆風半径を偶然ではなく明示にする。

残り 3 件はサーバ側の異常であり、修正ではなく `UNRESOLVABLE` としてラベルした。いずれも算術的に
唯一自然な読みであり、いずれも同型の受理済み challenge で twin 確認できた — 40.00 は
`06b428962fef92f5`（複数形の "claws" が第 1 operand を 2 倍しないことも同時に決着する）、5.00 と
23.00 は `d06a5d4a3beab3f1` — そのうえで却下されている。これで known-unresolvable は 2236 中 8 件
（0.36%）。8 件のうち 4 件が同じ「X and Y, total force」加算形になったのは、ランダムな flake では
なくサーバ側のパターンの可能性として観察に値する。

検証は第10 amendment の指示どおり、各ステップの前に `differential_replay.py` を回した。動いた
challenge 集合は毎回意図したものと完全に一致し、それ以外は動かなかった（合計 6 件）。ground-truth
ゲートは **unique 2236 / parsed 1836 (82.1%) / abstained 400 / correct 1828 / wrong 0 /
unlabeled 0 / known-unresolvable 8 — HARD GATE: PASS** に復帰した。correct は 1828 のまま不変で、
coverage が 82.4% → 82.1% に下がったのが支払った代価のすべてである。

レビューラウンドが、corpus には構造的に見えないものを捕らえた。新しいガードは 2 つとも当初 **生の
atom** を比較しており、このモジュールが他の全箇所で扱っている難読化層がそのまま脇をすり抜けていた:
二重字化が分割断片 `t` を `tt` にすると Arm B は発火を止め（`trween tt hree` が再び 8.00）、単位名詞が
2 つの出現で違う分割をされると（`* new tons ... newtons`）単位句ガードを抜けた（再び 512.00）。
どちらの形も corpus に存在しないので、どちらの replay ゲートでも発見できなかった — ライブトラフィック
から作ったゲートは**起きたこと**を証明するが、同じ生成器が**次に作れるもの**は証明しない。両者とも
collapse 後の形で比較するようにし、単位句の比較は各側で未使用 atom を最大 2 つまで merge する（cue や
operand が「単位名詞」に飲み込まれない上限つき）。両修正とも corpus 上で動いた challenge はゼロ —
純粋な精度向上である。python レビューは別途 Arm B のドキュメントを締めた（形と位置しか検査していない
のに tens+unit 複合語を検出すると称していた）。security レビューの指摘で、新ガードの最悪ケースを
複雑度クラスの類推ではなく直接踏む敵対的入力 fixture を 2 本追加した。

既存テスト 1 本がこの負けた側を固定していた。`"twenty five newtons and slows by seven newtons
what is total force"` を 18.00 に固定しており、根拠は「末尾の `total` は不活性な場面ノイズである」
— まさにサーバがその減算読みを却下した形である。コメントも第 6 amendment の書き直し以降存在しない
