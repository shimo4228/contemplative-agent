...
- 初回実行（2026-08-29）で書式の不連続点が生じる: A–E 縦断系列は終わり、境界をまたぐ
  ...
  不連続点は各文書にスタンプされる
- レポート系列の縦断読みには、それより前の境界が 2 つある（codemap の退役時に本 ADR へ移した記録、
  2026-09-05、ADR-0102）: **2026-08-16** — レポートセッションから user settings 層を落としたことで
  model / style が `claude-fable-5` / `Explanatory` から project 既定に変わり、この日以降に終わる
  レポートは別の計器であって、この日をまたぐ文体の変化は境界でありシグナルではない。
  **2026-08-29** — ADR-0098 によりレポートと診断が materials ファイル経由の 1 セッションから
  出るようになった（本 ADR の書式変更と同じ初回実行）。レポートの日本語訳は 2026-08-29 の境界で終わる
