...
5. **マーカーはレビュー提出時に進む（採用時ではない）。** `insight
   ...
   新パターン経由で必ず戻る。
6. **LLM novelty ゲート。**
   > **注記（2026-09-19, RFC-0042）**: 下記の fail-open 方針はそのまま有効だが、
   > その理由づけ（「誤って抑制されたテーマは二度と見えない」）は judge の
   > **tie-break** を支配しなくなった。この読みは同じ ADR の D5 / D9（実測の
   > 再発性 — 誤って却下された本物のテーマは新パターン経由で戻る）と食い違う。
   > そこから生まれたプロンプト文は下の Amendment（2026-09-19）が置き換える。
   > fail-open（LLM 失敗・解析不能時にクラスタを未判定で残す）は別の判断で、
   > 変更していない。
   クラスタリング後・抽出前に 1 回のグルーピング
...
## Amendment（2026-09-19）: judge の決定化と、covered 向きの tie-break

RFC-0042 作業項目 1。対象は novelty ゲートの判定コールと、そのプロンプト末尾の
tie-break だけ。D6 と 2026-07-18 Amendment の他の部分は変えない。

### Context

ゲートは `generate_full` の既定温度 1.0 で走っていた。2026-09-19 の run の
judge プロンプト 16 chunk を byte 一致で再生し、末尾の tie-break だけを変数にした
read-only 測定（[docs/evidence/rfc-0041/](../evidence/rfc-0041/README.md) 読み 1）の結果:

- t = 1.0 では**同一**プロンプトの 2 反復が covered 集合の半分しか共有しない
  （Jaccard 0.50）。総数は 154 中 66 / 66 で安定し、中身だけが入れ替わる — 成否を
  決めているのはプロンプトでなくサンプリングの揺れ。ゲートが却下した 68 件のうち
  17〜20 件は、同じ文言の再生で covered に変わった
- t = 0 では再生が決定的（反復間 Jaccard 1.0）で、現行文言のままでも covered は
  66 → 99 / 154
- tie-break が効くのは揺れが消えた後。t = 0 で 2 文の削除が 112、covered へ反転が
  127 / 154。t = 1.0 では削除アーム（80 / 65）が基準値を跨いでしまう

D6 が「迷ったら NEW」を正当化した根拠（誤って抑制されたテーマは二度と見えない）は、
同じ ADR の D5 / D9 と矛盾する — 却下や singleton 脱落が喪失でなく先送りである
理由が、まさにその実測された再発性だった。両立しない以上、実測のある側を採る。

### Decision

1. **判定コールは temperature 0**（`_NOVELTY_TEMPERATURE`。既定値の継承でなく
   明示的に渡し、値がコールの場所で読めるようにする）。再実行で評決が変わる
   ゲートはゲートではない。決定性は、evidence の再生が「サンプラーの測定」でなく
   「プロンプトの測定」になる条件でもある。
2. **プロンプトの tie-break を covered 向きにする。** `insight_novelty.md` の
   末尾 2 文を、迷ったら **already covered** に倒す文言に差し替える — 本物のテーマは
   後続の窓で再発し（D5 / D9）、重複スキルの代償は遅れたスキルより大きいため。
   文言は `covered` アームを測定したときの文字列そのもので、言い換えると判断が
   evidence から切り離される。これ以上は詰めない: 根拠は 1 run（154 クラスタ、
   2 週分の窓）であり、次の読みは再開後の実数（RFC-0042 の review-when）。
3. **fail-open は不変。** LLM 失敗・解析不能・予算超過はこれまでどおりクラスタを
   未判定で残す。変えたのは「答えた judge が迷いをどちらへ倒すか」であって、
   「答えが返らないときどうするか」ではない。
4. **監査行に温度を載せる。** `insight-novelty.jsonl` の judge レコードに
   `temperature` を足す（プロンプトを組まずコールもしない `fail_open_budget` は
   null）。このログは 2 つのサンプリングレジームに跨がることになり、区別できない
   リプレイは t = 0 の評決と t = 1.0 の評決を「プロンプトだけが違う」ものとして
   比べてしまう。欄の不在は 2026-09-19 以前の既定 1.0 を意味する。他の欄は不変なので
   既存レコードは引き続きリプレイ可能。

### Consequences

- 件数への見込み（同じ run のラベルのある部分集合）: 土曜ゲートへ届いた 75 件に
  対し、t = 0 + 反転 tie-break のゲート通過は 25 件（t = 0 + 現行文言なら 42 件）。
  154 クラスタ全体でのゲート通過は 27 対 55。
- ゲートは依然として**区別をしない**: どのアームも保留 7 件を却下 68 件と
  同等以上の率で止める（成果物の前に置いた judge には比べる証拠が無い — ADR-0084）。
  本 amendment が買うのは件数と再現性であって選別ではない。選別は RFC-0042 の作業項目 2〜4。
- judge が本物のテーマを誤って covered と呼んだ週は、レビュー枠でなく後続の窓への
  先送りを支払う — D5 / D9 が既に値付けしたトレードそのもの。
- 回帰: `tests/test_insight.py::TestFilterNovelBatches::test_judge_call_is_deterministic`
  がコール時の温度を、監査テストが新しい欄を固定する。

> **注記（2026-09-19, ADR-0111）**: 上で「selectivity は RFC-0042 項目 2〜4」と
> 書いた区別する力は、この gate の**内側でなく周囲**の段として実装された。gate と
> 抽出の間に名乗りコールが入り、候補が書かれた後に重複判定が入る。ラベルのある集団を
> 分けるのは後者だけである。この gate の役割は変わらない — 1 run の名乗り回数を抑える
> 安い batch の前絞り。D6 の「『同じテーマか』に embedding の分離は無い」という読みと
> fail-open の向きはどちらも生きている。
