# ADR-0090: 憲法改正の採択前に IPD 2 アームベンチを回す
...
insight 採用 5 バッチ（2026-05-05〜08-09 に 49 skill 承認、`audit.jsonl`
より）。3 ヶ月の空白と質的に異なるコーパスにより、次の改正は通常より
大きなジャンプになる — まさに行動面の読み値がコストに見合う局面である。

基準点の実験衛生（ADR-0056、変数は一度に一つ）: この改正は動いた唯一の
値層変数である — skill description 監査と name backfill は本改正との
重複を避けるため明示的に保留された（ADR-0081 Amendment 2026-08-08）。
隣接する機構層の変更が前日に 1 件ある: ADR-0081 enforcement フラグの
退役（`af1f607`、挙動変化なし）。T-P3 縦断読みは 2026-08-09 を「その
機構層フロアの上での、クリーンな値層の一段」として扱うこと。
...
初回実運用の前に null ペアを回した（2026-08-06/07、読みと生 run は
[docs/evidence/adr-0090/](../evidence/adr-0090/)）: **同一の**憲法
（sha256 `3d8a8503…` = 2026-05-05 承認レコードと一致）を n=10・本番モデル
（gemma4:e4b）で 2 回通した（3,076 秒と 4,082 秒 = 51〜68 分）。これに
より計器の較正規約が確定した:

- 方向（custom > baseline）は 6/6 セルで再現。α 勾配（相互性）も両 run
  で保存
- 主指標 Δ(custom − baseline) の run 間 noise floor は **±0.13** —
  ペアが出した α 別の揺れ 3 点（α = 0.0 / 0.5 / 1.0 で
  0.040 / 0.080 / 0.130）の最大値。単一ペアからの 3 観測なので floor
  自体に不確かさがあり、null ペア自身の但し書きも引き継ぐ — n=10 では
  run レベル相関シフトの検査は検出力が弱く、「見えなかった」は
  「無い」ではない
...
contemplative-ipd を staging と採択の間で **2 アーム比較**として回す:
arm A = 現行本番憲法、arm B = staged 改正案。**全面的な憲法改正は必ず
承認前にこのベンチを回す**（手順:
[docs/runbooks/constitution-amendment.md](../runbooks/constitution-amendment.md)）。
省略には ADR 系譜に記録された明示的なオーナー判断を要する。レポートは
**人間の承認パケットに添付**される。判断するのは人間であり、
パイプラインではない。

- `scripts/ipd-two-arm.sh` — オーケストレーション。staged の `.md` を
  特定し（ちょうど 1 件でなければ即失敗）、arm A の sha256 を audit
  ログの最後の **approved** amend-constitution ハッシュと照合し
  （ログは短縮ハッシュを保存するため prefix 照合。不一致は即失敗、
  bypass は非本番 home 向けの `AUDIT_CHECK_BYPASS=1` のみ）、来歴
  （両アームの sha256・モデル・n）を `provenance.txt` に記録し、
  JST 0/6/12/18 のスケジュール窓ガード（窓まで 75 分未満なら開始拒否、
  窓中は窓 + 60 分待機。非本番機向けの明示的な逃し弁として
  `SKIP_WINDOW_GUARD=1` がある）付きで両アームを順に実行し、レポートを
  出す。silent fallback は一切ない: ベンチ未インストール・憲法欠落・
  staged 件数 ≠ 1 は説明付きエラーで中断する。
...
読み値は採択を**ゲートしない**。承認ゲート付きコマンドは即応でなければ
...
n=10 **と生成モデル**は較正規約の一部である: ±0.13 floor は n=10 での
gemma4:e4b の分散の性質なので、`N_SIMS` **または** `OLLAMA_MODEL` を
変えると floor は無効になり、新しい null ペアが必要になる（wrapper が
両ノブの隣にこれを明記している）。レポートスクリプトは較正外の n や
α セル集合を即失敗させる。
...
- 2 アームベンチ: gemma4:e4b、n=10、paper protocol、窓ガード下で実行。
  レポート・来歴・生アーム出力は
  [docs/evidence/adr-0090/](../evidence/adr-0090/) に保存。
...
増える（n=40 ≈ 片アーム 4〜5 時間）。null ペアの読みが既に規則を
...
- 憲法の承認ゲートが、**事前登録された解釈規約**付きの行動面の読み値を
  ...
  ことができない（floor 自体の不確かさは Negative に記載）。
- 来歴が機械的になる: 両アームの sha256 が `provenance.txt` に残り、
  wrapper は実行前に arm A を audit ログの最終承認 content hash と
  照合する。（2026-08-09 の初回実運用ではこの照合は手動だった —
  本改訂で機械化した。）
...
- IPD 面が測るのは協力だけ。この計器に見えない面（例: 矛盾指示下の
  ...
  読んではならない。
- ±0.13 floor は**単一の** null ペアが出した α 別揺れ 3 点の最大値 —
  測定された分布ではなく、実質的な不確かさを持つ推定値である。2 組目の
  null ペアで締められるが、まだ実施していない。
- 探索的な AILuminate 面は、両憲法テキスト（値層）を第三者 API
  （Anthropic）へ test・judge コールとして送信した。既定の IPD 面は
  完全ローカルである。将来の非ローカル面はこの露出を繰り返すので、
  security by absence の立場と天秤にかけること。
...
（null ペア未実施）の探索的読みとしてのみ記録 — 読み値は
[docs/evidence/adr-0090/ailuminate-two-arm-report.md](../evidence/adr-0090/ailuminate-two-arm-report.md)、
後続は台帳 T-CONST-SAFETY-FACE。

探索的な数値（観察であり signal ではない — この計器の baseline 揺れは
チーム側 2026-04-13 の 4 run 実測で ~1.6 点、出典は commit `5242e74` の
`llm_benchmark_results/clause_variant_comparison.md`。run 間 floor は
未測定）: arm 内の baseline 超えは non_duality を除き両アームで正
（non_duality は両アームで負 — チームの「最弱 principle」所見と整合）。
アーム間の最大の動き 2 つは同じ大きさで逆方向だった: combined 技法の
持ち上げは弱まり（arm A +5.9 → arm B +1.4）、non_duality の平均は改善
した（55.7 → 61.6、+5.9）。どちらも未較正の計器上では読めない。平均から
除外した judge 失敗は arm A 9/300、arm B 15/300。
...
なりえた。実際には起きていない: 採択は 21:10 JST でスケジュール
セッション（JST 0/6/12/18）の間にあり、`api-audit.jsonl` は 12:00〜14:00
UTC の窓で API コール 0 件 — 二重状態を読んだセッションは無い。数分
以内に手動修復（旧文面を evidence dir へ退避、新文面を正位置へ移動、
sha を audit 記録と照合）。恒久的に残る不整合が 1 点ある: append-only の
audit ログの承認レコードは既に存在しないパス
`contemplative-axioms-2.md` を指す — 信頼できるキーは content hash で
あり、パスではない。このガードは skills には正しい（clobber = データ
損失）が、単一ファイル置換ターゲットには誤り — 台帳タスク
T-ADOPT-OVERWRITE-TARGETS で追跡。

## References

- [docs/evidence/adr-0090/](../evidence/adr-0090/) — null ペアの読み +
  生 run、2 アームレポート + 来歴 + 生アーム、退役した 2026-05-05 憲法、
  AILuminate 探索レポート
