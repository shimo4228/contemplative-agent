...
The agent plist template (`config/launchd/com.moltbook.agent.plist`) bakes `DECISION_MODEL=gemma4:e4b` and `DECISION_FACES=relevance` into `EnvironmentVariables` (RFC-0046 / ADR-0113, owner decision 2026-09-25) because launchd does not inherit shell exports; re-running `install-schedule` re-renders the template, so the shadow survives a reinstall, and removing the two keys from the template is how it is turned off. `install-schedule` manages the agent, distill, insight, backup, weekly-pipeline, and watchdog plists — declaratively: re-running it removes any optional job whose flag is not passed, so always pass the full desired set. One plist under `config/launchd/` is outside its scope: `com.moltbook.ollama-restart.plist` is installed and updated manually via `launchctl`, and `--uninstall` does not touch it. A legacy `com.moltbook.weekly-analysis` job is likewise outside its scope — the standalone installer was removed on 2026-08-29, so if that plist still exists on a machine it is no longer managed and must be removed by hand (`launchctl unload` + `rm`). It restarts the local `ollama serve` nightly at 23:55 — starting `ollama serve` directly (no GUI app) with `AbandonProcessGroup` so the daemon survives its parent exiting. Between the `pkill` and the restart it calls `scripts/rotate-log.sh <path> 7`, which moves `ollama-serve.log` aside and gzips it, keeping seven compressed generations: that is the one moment in the day when the writer is dead, so the job that owns the log also rotates it. Rotation failures are chained with `;` so they can never stop the daemon from starting, and are prefixed `ERROR:` so the weekly anomaly sweep sees them. `RunAtLoad` is set, so `launchctl load` restarts Ollama on the spot — reload it outside a scheduled session window. The repo copy is the reference template: when the live plist changes, mirror it here in the same change.

`scripts/rotate-log.sh` has a second caller: `scripts/backup-runtime.sh` rotates `agent-launchd.log` into eight generations at the top of the weekly backup. Nothing kills that log's writer first — a live agent session may hold it open — so the script's `lsof` open-writer check is the whole of the safety there, and a backup landing inside a session window skips the rotation loudly rather than truncating the session's own evidence. That check needs `lsof`, which macOS keeps in `/usr/sbin`: `com.moltbook.backup.plist` pins `PATH` and must therefore include `/usr/sbin`, and the script independently retries `/usr/sbin/lsof` when `command -v` misses (RFC-0030 — without either, every weekly rotation ran unguarded). The plist half only reaches the machine on the next `install-schedule --weekly-backup`; until then the script's retry is what is actually protecting the log.
