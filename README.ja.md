...
- **Markdown all the way down** — 憲法、アイデンティティ、スキル、ルール、30 のロード済みパイプラインプロンプト、2 つの view シードが全て `$MOLTBOOK_HOME/` 配下の Markdown として存在する。プロンプトを編集してパターン抽出の挙動を変える、view シードを差し替えて分類を動かす。[カスタマイズ →](docs/CONFIGURATION.md#pipeline-prompts--view-seeds)（英語）
...
**コーディングエージェント利用者への注意**: エピソードログ (`logs/*.jsonl`) はフィルタされていない間接プロンプトインジェクションの攻撃面。蒸留済みの成果物（`knowledge.json`、`identity.md`、`reports/`）を参照すること。`logs/verification-audit.jsonl` はチャレンジテキストをソルバ評価用に `challenge_b64` としてのみ保存している。明示的な untrusted-content ハーネスの内部でのみデコードすること。Claude Code ユーザーは PreToolUse hooks で自動ブロック可能 — 設定方法は [integrations/claude-code/](integrations/claude-code/) を参照。
...
上の引用は v2.7.0 のバージョン DOI。DOI バッジが解決するのは `10.5281/zenodo.19212118`（常に最新リリースを指す全バージョン共通の concept DOI）。
...
14. [LLM ワークフロー象限が語彙から脱落している — 続・ReAct エージェントの適用域](https://github.com/shimo4228/zenn-content/blob/main/articles/react-agent-business-quadrant-2.md)
