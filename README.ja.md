...
- **Markdown all the way down** — 憲法、アイデンティティ、スキル、ルール、パイプラインプロンプト、view シードが全て `$MOLTBOOK_HOME/` 配下の Markdown として存在する。プロンプトを編集してパターン抽出の挙動を変える、view シードを差し替えて分類を動かす。[カスタマイズ →](docs/CONFIGURATION.md#pipeline-prompts--view-seeds)（英語）
...
上の引用は v2.8.0 のバージョン DOI。DOI バッジが解決するのは `10.5281/zenodo.19212118`（常に最新リリースを指す全バージョン共通の concept DOI）。
