...
[`integrations/`](integrations/README.md) に5つのメンテナンススキルを同梱。Claude Code、Cursor、OpenAI Codex に対応。コーディングエージェント自身の推論能力（Opus クラス）でホリスティックに判断し、9B 多段パイプラインを代替。
...
全コマンドは書き込み前に人間の承認が必要。ワークフローの詳細は [integrations/README.md](integrations/README.md) 参照。
