...
このリポジトリを [Claude Code](https://claude.ai/claude-code)（または `.claude/skills/` を読むコーディングエージェント）で開くと、5つのスラッシュコマンドが使える。コーディングエージェント自身の推論能力（Opus クラス）で行動成果物をメンテナンスする — 9B 多段パイプラインをホリスティック判断で代替。

| コマンド | AKC フェーズ | 内容 |
|---------|------------|------|
| `/insight-ca` | Extract/Curate | knowledge.json (uncategorized) → skills/*.md |
| `/skill-stocktake-ca` | Curate | skills/ + rules/ の重複・陳腐化を監査 |
| `/rules-distill-ca` | Promote | skills/*.md → rules/*.md（横断的原則の抽出） |
| `/amend-constitution-ca` | Promote | knowledge.json (constitutional) → constitution/*.md |
| `/distill-identity-ca` | Promote | knowledge.json + identity.md → 更新版 identity.md |

全コマンドは書き込み前に人間の承認が必要。`-ca` サフィックスはコーディングエージェント自身のメンテナンススキル（learn-eval, skill-stocktake, rules-distill）と区別するため。

**セキュリティ**: `knowledge.json`（サニタイズ済み）のみ読む。生のエピソードログ（`logs/*.jsonl`）は読まない — [ADR-0007](docs/adr/0007-security-boundary-model.md) 参照。

**前提条件**: 先に `contemplative-agent distill` を実行して knowledge.json を生成しておくこと。
