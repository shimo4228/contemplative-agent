...
Windowing and the log walk are :mod:`.selection_window`'s, shared with
:mod:`.selection_metrics` and :mod:`.skill_confusion`, so the readings of
one log cannot disagree about where the window is or what was in it.
"""
...
# The withholding decision reads the reason codes AFTER they are filtered
...
def _split_populations(
    catalog_names: list[str], tally: _SelectionHistoryTally, exposure_floor: int
) -> tuple[list[NeverSelectedSkill], list[NeverSelectedSkill], list[NeverSelectedSkill]]:
    """Sort the catalog into ``(strict, dormant, below_floor)``."""

    def _entry(name: str) -> NeverSelectedSkill:
        return NeverSelectedSkill(
            name=name,
            judged_exposure=tally.exposure_history.get(name, 0),
            window_exposure=tally.exposure_window.get(name, 0),
            last_selected=tally.last_selected.get(name, ""),
        )
    ...
    for name in catalog_names:
        if name in tally.selected_history:
            ...
            if name not in tally.selected_window and tally.exposure_window.get(name, 0) > 0:
    ...
    return strict, dormant, below_floor


def _never_selected_reasons(
    tally: _SelectionHistoryTally,
    *,
    catalog_available: bool,
    has_strict: bool,
    has_below_floor: bool,
) -> tuple[str, ...]:
...
def read_never_selected(
    ...
    **Two scopes in one pass, on purpose.** The strict population is defined
    ...
    reasons = _never_selected_reasons(
        ...
        has_below_floor=bool(below_floor),
    )
    flagged = set(reasons)
    ...
    return NeverSelectedReading(
        ...
        history_fail_open=tally.history_fail_open,
        history_first_day=min(tally.days_read) if tally.days_read else "",
        ...
        history_full_skill_tokens=tally.full_skill_tokens,
        num_ctx=NUM_CTX,
